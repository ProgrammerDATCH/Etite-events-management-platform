...
import { commonStyleClasses, isEmailValid, isStrongPassword } from '../../components/common'
...
export const AddUsers = () => {
  ...
  async function handleRegister(e) {
    ...
    if (status) {
      toast.success(`${message.names} Registered successfully!`);
      navigation('/admin/users?new=added');
    }
    else {
      toast.error(message);
    }
  ...
  return (
    <div className={commonStyleClasses("courses", null ,true).parentDiv}>
      <h1 className='text-5xl mb-10 text-center'>Add User</h1>
      <form onSubmit={handleRegister} className='flex flex-col justify-center items-center'>
        ...
        <input
          ...
          onChange={(e) => { setRegisterData({ ...registerData, phone: e.target.value }); setErr((prevErr) => ({ ...prevErr, phone: false })); }}
          className={`${commonStyleClasses(true, err.phone).inputText}`}
        />
        <input
          type="password"
